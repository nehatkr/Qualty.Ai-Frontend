...
export default function ContactPage() {
 ...
 try {
        ...
        if (!response.ok) {
            const errorMessage = result.error || 'Submission failed due to an unknown server error.';
  ...
  const renderFormContent = () => {
    return (
        <div className="w-full bg-neutral-900 rounded-xl p-8 shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {errors.api && (
                    <div className="p-3 bg-red-900 border border-red-700 text-red-300 rounded-lg flex items-center">
                ...
                <button
                    type="submit"
                    disabled={isLoading}
                    className="mt-4 w-full bg-white text-black py-2 rounded font-medium hover:bg-gray-200 transition disabled:opacity-50 flex justify-center items-center"
                >
                    {isLoading ? <Loader2 className="animate-spin w-5 h-5 mr-2" /> : <Send className="w-4 h-4 mr-2" />}
