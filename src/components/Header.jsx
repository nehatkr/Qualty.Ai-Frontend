...
const Header = ({ user, onLogout }) => { // Remove onAuthClick prop
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const navigate = useNavigate(); // Add this hook

  // Handle auth click directly in the component
  const handleAuthClick = () => {
    navigate('/login');
  };

  // ... rest of your headerStyles object stays the same ...
  const headerStyles = {
    header: {
      position: 'fixed',
      top: 0,
      ...
      <div style={headerStyles.headerContent} className="header-content">
        ...
        <nav style={headerStyles.nav} className="header-nav">
          ...
          {user ? (
            ...
            <button
              onClick={handleAuthClick} // Use the local function instead of onAuthClick prop
              style={headerStyles.ctaButton}
              onMouseOver={(e) => {
                e.target.style.transform = 'translateY(-2px)';
                e.target.style.background = 'rgba(255, 255, 255, 0.2)';
      ...
      >
        <div style={headerStyles.mobileMenuContent}>
          ...
          {user ? (
            ...
            <button
              onClick={() => {
                handleAuthClick(); // Use the local function
                setIsMenuOpen(false);
              }}
              style={headerStyles.mobileCTAButton}
              onMouseOver={(e) => {
